...
const BlogDetail: React.FC = () => {
  ...
  const loadBlog = async () => {
    ...
    try {
      ...
      const { userMessage } = handleApiError(error, 'loading blog content');
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      setBlog(prev => (prev ? { ...prev, _count: { ...prev._count, likes: response.likeCount } } : null));
      toast.success(response.liked ? 'Post liked!' : 'Like removed.');
    } catch (error) {
      const { userMessage } = handleApiError(error, 'liking the post');
      toast.error(userMessage);
    } finally {
  ...
  const handleBookmark = async () => {
    ...
    try {
      ...
      toast.success(response.bookmarked ? 'Post bookmarked!' : 'Bookmark removed.');
    } catch (error) {
      const { userMessage } = handleApiError(error, 'bookmarking the post');
      toast.error(userMessage);
    } finally {
      setActionLoading(null);
    }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">The blog you are looking for does not exist or has been moved.</p>
