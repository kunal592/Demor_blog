...
const CreateBlog: React.FC = () => {
  ...
  const { user } = useAuth(); // Keep this line
  ...
  const handleSubmit = async (published: boolean) => {
    ...
    setSaving(true);
    try {
      const blogData: BlogFormData = { ...formData, isPublished: published };
      const newBlog = await blogService.createBlog(blogData);
      toast.success(published ? 'Blog published! 🎉' : 'Blog saved as draft 📝');
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Create a New Blog</h1>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <div className="space-y-3">
                <button onClick={() => handleSubmit(true)} disabled={saving} className="w-full flex items-center justify-center px-4 py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-medium rounded-lg">
                  {saving ? <div className="animate-spin w-4 h-4 border-2 border-white border-t-transparent rounded-full mr-2"></div> : <Save className="h-4 w-4 mr-2" />} Publish
                </button>
